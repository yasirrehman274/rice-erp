...
export default function SaleForm({ sale }: { sale?: Sale }) {
  ...
  const totals = useMemo(() => {
    ...
    return {
      subtotal,
      grandTotal: Math.max(0, subtotal - discount + shipping + other),
    };
  }, [
    values.items,
    values.discount,
  ...
  function stockOf(productId: string): number {
    if (!productId || !values.warehouseId) return 0;
    return (
      inventory.find(
        (item) =>
  ...
  function effectiveStock(item: SaleItemForm): number {
    if (!item.productId) return 0;
    const total = stockOf(item.productId);
    const otherRows = values.items
      .filter((i) => i.productId === item.productId && i.id !== item.id)
      .reduce((sum, i) => sum + (Number(i.quantity) || 0), 0);
    return Math.max(0, total - otherRows);
  }

  const stockErrors = useMemo(
    () =>
      saleService.validateItemStock(
        values.items,
        values.warehouseId,
        (productId) => {
          if (!values.warehouseId) return 0;
          return (
            inventory.find(
              (item) =>
                item.productId === productId &&
    ...
    [values.items, values.warehouseId, inventory],
  );

  const stockErrorMap = useMemo(() => {
    const map: Record<string, string> = {};
    for (const error of stockErrors) map[error.itemId] = error.message;
    return map;
  }, [stockErrors]);
  ...
  function buildErrors(): Record<string, string> {
    ...
    if (!values.warehouseId)
      nextErrors.warehouseId = "Dispatch Warehouse is required.";
    ...
    if (invalidIndex !== -1) {
      ...
      nextErrors.items = `Item ${invalidIndex + 1} needs a product, quantity > 0, and valid price.`;
      if (!item.productId)
        nextErrors[`item-${item.id}-product`] = "Select a product.";
    }
    const stockIssues = saleService.validateItemStock(
      values.items,
      values.warehouseId,
      (productId) => {
        if (!values.warehouseId) return 0;
    ...
    if (stockIssues.length > 0) {
      nextErrors.items =
        "Cannot complete sale: Requested quantity exceeds available warehouse stock.";
      for (const issue of stockIssues)
        nextErrors[`item-${issue.itemId}-stock`] = issue.message;
    }
    ...
    if ((Number(values.receivedAmount) || 0) > totals.grandTotal)
      nextErrors.receivedAmount = "Received amount cannot exceed grand total.";
    return nextErrors;
  }
    ...
    <form onSubmit={submit} className="mx-auto max-w-7xl space-y-8">
      ...
      <section className={cardContainer}>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          ...
          <label className="block">
            <span className="mb-2 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
              Seller Terminal <span className="text-rose-500">*</span>
            </span>
            ...
            <div className="divide-y divide-slate-100 bg-white dark:divide-slate-800/60 dark:bg-slate-900/40">
              {values.items.map((item, index) => {
                const itemStockError =
                  stockErrorMap[item.id] ?? errors[`item-${item.id}-stock`];
...
//   const stockErrors = useMemo(
//     () =>
//       saleService.validateItemStock(values.items, values.warehouseId, (productId) => {
//         if (!values.warehouseId) return 0;
//         return (
//           inventory.find(
//             (item) =>
...
//     }
//     const stockIssues = saleService.validateItemStock(values.items, values.warehouseId, (productId) => {
//       if (!values.warehouseId) return 0;
//       return (
//         inventory.find(
//           (item) => item.productId === productId && item.warehouseId === values.warehouseId,
//         )?.availableStock ?? 0
//       );
//     });
//     if (stockIssues.length > 0) {
//       nextErrors.items = "Sale cannot be completed because requested quantity exceeds available stock.";
//       for (const issue of stockIssues) nextErrors[`item-${issue.itemId}-stock`] = issue.message;
//     }
//     if ((Number(values.receivedAmount) || 0) < 0)
//       nextErrors.receivedAmount = "Received amount cannot be negative.";
//     if ((Number(values.receivedAmount) || 0) > totals.grandTotal)
//       nextErrors.receivedAmount = "Received amount cannot exceed the grand total.";
//     return nextErrors;
//   }
...
//           </div>
//           {values.items.map((item, index) => {
//             const itemStockError = stockErrorMap[item.id] ?? errors[`item-${item.id}-stock`];
//             const available = effectiveStock(item);
//             return (
//               <div
//                 key={item.id}
...
//                       .filter((product) => product.status === "active")
//                       .map((product) => {
//                         const outOfStock = values.warehouseId !== "" && stockOf(product.id) <= 0;
//                         return (
...
//                     Stock
//                   </span>
//                   <p className={`text-sm font-medium ${item.productId && available === 0 ? "text-rose-600" : ""}`}>
//                     {available.toLocaleString()}
//                   </p>
//                 </div>
//                 <div>
...
//                       updateItem(index, "quantity", event.target.value)
//                     }
//                     className={`${inputClass} ${itemStockError ? "border-rose-400 ring-4 ring-rose-500/10 focus:border-rose-500" : ""}`}
//                   />
//                   {itemStockError && (
//                     <span className="mt-1 block text-xs text-rose-600">
//                       {itemStockError}
...
//             type="submit"
//             disabled={hasStockErrors}
//             title={hasStockErrors ? "Reduce the quantity to the available stock before saving." : undefined}
//             className="mt-8 inline-flex h-13 w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-5 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-50"
//           >
//             <Save size={17} />
