...
export default function SaleLedger({ sale, entries }: { sale: Sale; entries: LedgerEntry[] }) {
  ...
  return <div className="space-y-6">
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div><p className="text-sm text-slate-500">Customer ledger</p><h2 className="mt-1 text-xl font-bold">{sale.customerName}</h2><p className="mt-1 text-sm text-slate-500">Sale: {sale.saleNumber}</p></div>
