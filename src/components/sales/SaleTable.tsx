...
export default function SaleTable({ initialSales }: { initialSales: Sale[] }) {
  ...
  function handleDelete() {
    if (!deleting) return;
    saleService.delete(deleting.id);
    setSales((current) => current.filter((s) => s.id !== deleting.id));
    setDeleting(null);
  }
  ...
  return (
    <>
        ...
        <div className="flex flex-col gap-3 border-b border-slate-100 p-4 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-2">
            ...
            <select
              ...
              onChange={(event) => {
                setPaymentStatus(
                  event.target.value as "all" | SalePaymentStatus,
                );
                setPage(1);
              }}
        ...
        {visible.length === 0 ? (
          <div className="grid min-h-80 place-items-center p-8 text-center">
            <div>
              ...
              <h2 className="mt-4 font-semibold">No sales found</h2>
                ...
                <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500 dark:bg-slate-800/40">
                  <tr>
                    <th
                      ...
                      onClick={() => changeSort("saleNumber")}
                    >
                      Sale No{sortIcon("saleNumber")}
                    </th>
                    <th
                    ...
                    <th
                      className="cursor-pointer px-4 py-3 select-none"
                      onClick={() => changeSort("saleDate")}
                    >
                      Date{sortIcon("saleDate")}
                    </th>
      ...
      <DeleteSaleDialog
        saleNumber={deleting?.saleNumber ?? ""}
        open={deleting !== null}
