...
interface PurchasePaymentDialogProps { purchase: Purchase | null; open: boolean; onClose: () => void; onConfirm: (amount: number, method: PurchasePaymentMethod, notes: string) => void; }

export default function PurchasePaymentDialog({ purchase, open, onClose, onConfirm }: PurchasePaymentDialogProps) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/50 p-4" role="dialog" aria-modal="true"><div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl dark:bg-slate-900">
    ...
    <p className="mt-2 text-sm text-slate-500">Payment for <span className="font-semibold text-slate-700 dark:text-slate-200">{purchase.purchaseNumber}</span>.</p>
    <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-500/20 dark:bg-amber-500/10">
      ...
      <p className="mt-1 text-xl font-bold text-amber-700 dark:text-amber-400">Rs. {new Intl.NumberFormat("en-PK").format(purchase.remainingBalance)}</p>
    </div>
    <form onSubmit={submit} className="mt-5 space-y-4">
      <label><span className="mb-2 block text-sm font-medium">Amount <span className="text-rose-600">*</span></span><input type="number" min="1" max={purchase.remainingBalance} value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="0" className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 dark:border-slate-700 dark:bg-slate-800" /></label>
      <label><span className="mb-2 block text-sm font-medium">Payment method</span><select value={method} onChange={(event) => setMethod(event.target.value as PurchasePaymentMethod)} className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-emerald-500 dark:border-slate-700 dark:bg-slate-800"><option value="cash">Cash</option><option value="bank">Bank Transfer</option><option value="cheque">Cheque</option><option value="online">Online</option></select></label>
