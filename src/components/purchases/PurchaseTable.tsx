...
export default function PurchaseTable({
  ...
  function handleDelete() {
    if (!deleting) return;
    purchaseService.delete(deleting.id);
    setPurchases((current) => current.filter((p) => p.id !== deleting.id));
    setDeleting(null);
  }
  ...
  const sortIcon = (key: SortKey) => (
    <ChevronsUpDown
      size={14}
      className={`ml-1 inline ${sort === key ? "text-emerald-600" : ""}`}
    />
  );
  ...
  return (
    <>
        ...
        <div className="flex flex-col gap-3 border-b border-slate-100 p-4 dark:border-slate-800 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative w-full sm:max-w-sm">
            ...
            <input
              ...
              className="h-10 w-full rounded-xl border border-slate-200 bg-slate-50 pl-10 pr-3 text-sm outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 dark:border-slate-700 dark:bg-slate-800"
            />
          </div>
          <div className="flex gap-2">
            ...
            <select
              ...
              onChange={(event) => {
                setPaymentStatus(
                  event.target.value as "all" | PurchasePaymentStatus,
                );
                setPage(1);
              }}
        ...
        {visible.length === 0 ? (
          <div className="grid min-h-80 place-items-center p-8 text-center">
            <div>
              ...
              <h2 className="mt-4 font-semibold">No purchases found</h2>
                ...
                <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500 dark:bg-slate-800/40">
                  <tr>
                    <th
                      ...
                      onClick={() => changeSort("purchaseNumber")}
                    >
                      Purchase No{sortIcon("purchaseNumber")}
                    </th>
                    <th
                    ...
                    <th
                      className="cursor-pointer px-4 py-3 text-right select-none"
                      onClick={() => changeSort("grandTotal")}
                    >
                      Total Amount{sortIcon("grandTotal")}
                    </th>
                    <th className="px-4 py-3 text-right">Paid</th>
                    <th
                      className="cursor-pointer px-4 py-3 text-right select-none"
                      onClick={() => changeSort("remainingBalance")}
                    >
                    ...
                    <th
                      className="cursor-pointer px-4 py-3 select-none"
                      onClick={() => changeSort("purchaseDate")}
                    >
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          {purchase.status === "pending" && (
                            <Link
                              href={`/purchases/receive/${purchase.id}`}
            ...
            <div className="flex flex-col items-center justify-between gap-3 border-t border-slate-100 px-4 py-4 dark:border-slate-800 sm:flex-row">
              ...
              <div className="flex gap-1">
                <button
                  ...
                  className="rounded-lg border border-slate-200 px-3 py-1.5 text-sm font-semibold hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40 dark:border-slate-700 dark:hover:bg-slate-800"
                >
                  Previous
                </button>
                {Array.from({ length: pageCount }, (_, i) => (
                  <button
                    key={i + 1}
                    onClick={() => setPage(i + 1)}
                    className={`size-9 rounded-lg text-sm font-semibold ${safePage === i + 1 ? "bg-emerald-600 text-white" : "border border-slate-200 hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-800"}`}
                  >
                    {i + 1}
                  </button>
                ))}
      ...
      <DeletePurchaseDialog
        purchaseNumber={deleting?.purchaseNumber ?? ""}
        open={deleting !== null}
