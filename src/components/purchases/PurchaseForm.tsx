...
function toFormValues(purchase?: Purchase): PurchaseFormValues {
  return purchase ? {
    purchaseNumber: purchase.purchaseNumber, purchaseDate: purchase.purchaseDate, supplierId: purchase.supplierId, warehouseId: purchase.warehouseId, productId: purchase.productId, batchNumber: purchase.batchNumber, riceVariety: purchase.riceVariety, quantity: String(purchase.quantity), bagWeight: String(purchase.bagWeight), totalWeight: String(purchase.totalWeight), purchaseRate: String(purchase.purchaseRate), subtotal: String(purchase.subtotal), discount: String(purchase.discount), transportCharges: String(purchase.transportCharges), otherCharges: String(purchase.otherCharges), grandTotal: String(purchase.grandTotal), paidAmount: String(purchase.paidAmount), paymentMethod: purchase.paymentMethod, status: purchase.status, notes: purchase.notes,
  } : emptyValues;
}
...
export default function PurchaseForm({ purchase }: { purchase?: Purchase }) {
  ...
  useEffect(() => {
    if (!purchase) {
      setValues((current) => ({ ...current, purchaseNumber: current.purchaseNumber || generatePurchaseNumber(), purchaseDate: current.purchaseDate || new Date().toISOString().slice(0, 10) }));
    }
  }, [purchase]);
  ...
  function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!values.purchaseNumber.trim()) nextErrors.purchaseNumber = "Purchase number is required.";
  ...
  if (saved) {
    return <div className="grid min-h-60 place-items-center rounded-2xl border border-slate-200 bg-white p-8 text-center shadow-sm dark:border-slate-800 dark:bg-slate-900"><div><div className="mx-auto grid size-14 place-items-center rounded-full bg-emerald-100 text-emerald-600 dark:bg-emerald-500/15"><Save size={26} /></div><h2 className="mt-4 text-lg font-bold">Purchase saved successfully</h2><p className="mt-1 text-sm text-slate-500">Redirecting...</p></div></div>;
  }
  ...
  return <form onSubmit={submit} className="space-y-6">
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6">
      <div className="mb-6"><h2 className="font-semibold">Purchase information</h2><p className="mt-1 text-sm text-slate-500">Enter the purchase details and invoice information.</p></div>
      <div className="grid gap-5 md:grid-cols-2">
        <label><span className="mb-2 block text-sm font-medium">Purchase Number <span className="ml-1 text-rose-600">*</span></span><input value={values.purchaseNumber} onChange={(event) => update("purchaseNumber", event.target.value)} placeholder="PUR-1085" className={`h-11 w-full rounded-xl border bg-white px-3 text-sm outline-none focus:ring-4 dark:bg-slate-800 ${errors.purchaseNumber ? "border-rose-500 focus:border-rose-500 focus:ring-rose-500/10" : "border-slate-200 focus:border-emerald-500 focus:ring-emerald-500/10 dark:border-slate-700"}`} />{errors.purchaseNumber && <span className="mt-1.5 block text-xs text-rose-600">{errors.purchaseNumber}</span>}</label>
        <label><span className="mb-2 block text-sm font-medium">Purchase Date <span className="ml-1 text-rose-600">*</span></span><input type="date" value={values.purchaseDate} onChange={(event) => update("purchaseDate", event.target.value)} className={`h-11 w-full rounded-xl border bg-white px-3 text-sm outline-none focus:ring-4 dark:bg-slate-800 ${errors.purchaseDate ? "border-rose-500 focus:border-rose-500 focus:ring-rose-500/10" : "border-slate-200 focus:border-emerald-500 focus:ring-emerald-500/10 dark:border-slate-700"}`} />{errors.purchaseDate && <span className="mt-1.5 block text-xs text-rose-600">{errors.purchaseDate}</span>}</label>
        <label><span className="mb-2 block text-sm font-medium">Supplier <span className="ml-1 text-rose-600">*</span></span><select value={values.supplierId} onChange={(event) => update("supplierId", event.target.value)} className={`h-11 w-full rounded-xl border bg-white px-3 text-sm outline-none focus:ring-4 dark:bg-slate-800 ${errors.supplierId ? "border-rose-500 focus:border-rose-500 focus:ring-rose-500/10" : "border-slate-200 focus:border-emerald-500 focus:ring-emerald-500/10 dark:border-slate-700"}`}><option value="">Select supplier</option>{suppliers.filter((s) => s.status === "active").map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select>{errors.supplierId && <span className="mt-1.5 block text-xs text-rose-600">{errors.supplierId}</span>}</label>
        ...
        <label><span className="mb-2 block text-sm font-medium">Batch Number</span><input value={values.batchNumber} onChange={(event) => update("batchNumber", event.target.value)} placeholder={generateBatchNumber()} className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 dark:border-slate-700 dark:bg-slate-800" /></label>
        ...
        {selectedSupplier && <div className="grid grid-cols-2 gap-4 md:col-span-2"><article className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/50"><p className="text-xs text-slate-500">Supplier Balance</p><p className="mt-1 text-lg font-bold">Rs. {new Intl.NumberFormat("en-PK").format(selectedSupplier.currentBalance)}</p></article><article className="rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/50"><p className="text-xs text-slate-500">Credit Limit</p><p className="mt-1 text-lg font-bold">Rs. {new Intl.NumberFormat("en-PK").format(selectedSupplier.creditLimit)}</p></article></div>}
      </div>
    </section>
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6">
      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <label><span className="mb-2 block text-sm font-medium">Purchase Rate (per KG) <span className="ml-1 text-rose-600">*</span></span><input type="number" min="0" value={values.purchaseRate} onChange={(event) => update("purchaseRate", event.target.value)} placeholder="0" className={`h-11 w-full rounded-xl border bg-white px-3 text-sm outline-none focus:ring-4 dark:bg-slate-800 ${errors.purchaseRate ? "border-rose-500 focus:border-rose-500 focus:ring-rose-500/10" : "border-slate-200 focus:border-emerald-500 focus:ring-emerald-500/10 dark:border-slate-700"}`} />{errors.purchaseRate && <span className="mt-1.5 block text-xs text-rose-600">{errors.purchaseRate}</span>}</label>
      </div>
    </section>
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-6">
      <div className="mb-6"><h2 className="font-semibold">Payment details</h2><p className="mt-1 text-sm text-slate-500">Record the payment information for this purchase.</p></div>
      <div className="grid gap-5 md:grid-cols-2">
        <label><span className="mb-2 block text-sm font-medium">Paid Amount</span><input type="number" min="0" max={values.grandTotal} value={values.paidAmount} onChange={(event) => update("paidAmount", event.target.value)} placeholder="0" className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 dark:border-slate-700 dark:bg-slate-800" /></label>
        ...
        {Number(values.grandTotal) > 0 && <div className="md:col-span-2"><article className="rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-500/20 dark:bg-amber-500/10"><p className="text-sm font-medium text-amber-700 dark:text-amber-400">Remaining Balance</p><p className="mt-1 text-2xl font-bold text-amber-700 dark:text-amber-400">Rs. {new Intl.NumberFormat("en-PK").format((Number(values.grandTotal) || 0) - (Number(values.paidAmount) || 0))}</p></article></div>}
        <label className="md:col-span-2"><span className="mb-2 block text-sm font-medium">Status</span><select value={values.status} onChange={(event) => update("status", event.target.value)} className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm outline-none focus:border-emerald-500 dark:border-slate-700 dark:bg-slate-800"><option value="pending">Pending</option><option value="received">Received</option><option value="partial">Partial</option><option value="cancelled">Cancelled</option></select></label>
