...
const size = 8;
type Sort = "name" | "city" | "currentBalance" | "createdAt";
...
export default function CustomerTable({
  ...
  const [prevInitial, setPrevInitial] = useState(initialCustomers);
  if (prevInitial !== initialCustomers) {
    setPrevInitial(initialCustomers);
  ...
  const rows = results.slice((current - 1) * size, current * size);
  function sortBy(key: Sort) {
    if (sort === key) setUp(!up);
    else {
      setSort(key);
      setUp(true);
    }
  ...
  return (
    <>
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-col gap-3 border-b border-slate-100 p-4 dark:border-slate-800 sm:flex-row sm:justify-between">
          <div className="relative w-full sm:max-w-sm">
            ...
            <input
              ...
              className="h-10 w-full rounded-xl border border-slate-200 bg-slate-50 pl-10 pr-3 text-sm outline-none focus:border-emerald-500 dark:border-slate-700 dark:bg-slate-800"
            />
          </div>
          <select
            value={status}
        ...
        {rows.length ? (
              ...
              <table className="w-full min-w-[1280px] text-left text-sm">
                <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500 dark:bg-slate-800/40">
                  <tr>
                    {[
                      ...
                      <th
                        key={label as string}
                        className="px-3 py-3 first:pl-6"
                      >
                        {key ? (
                          <button
                            onClick={() => sortBy(key as Sort)}
                ...
                <tbody>
                  {rows.map((item) => (
                    ...
                    >
                      <td className="px-3 py-4 pl-6 font-semibold">
                        <Link
                          href={`/customers/view/${item.id}`}
                      ...
                      <td className="px-3 py-4">
                        <span
                          className={`rounded-full px-2.5 py-1 text-xs font-semibold ${item.status === "active" ? "bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400" : "bg-slate-100 text-slate-600 dark:bg-slate-800"}`}
                        >
                          {item.status}
                        </span>
                      </td>
          ...
          <div className="grid min-h-80 place-items-center p-8 text-center">
            <div>
              ...
              <h2 className="mt-4 font-semibold">No customers found</h2>
        ...
        <div className="flex flex-col gap-3 border-t border-slate-100 px-5 py-4 text-sm dark:border-slate-800 sm:flex-row sm:justify-between">
          <p className="text-slate-500">
            Showing {results.length ? (current - 1) * size + 1 : 0}–
            {Math.min(current * size, results.length)} of {results.length}{" "}
            customers
          </p>
      ...
      <DeleteCustomerDialog
        ...
        onConfirm={() => {
          if (deleting) {
            customerService.delete(deleting.id);
            setCustomers((items) =>
